...
const Skills = () => {
  const front = frontend.map((elem) => {
    return <DisplayBox key={elem.id} name="Frontend" item={elem} />
  })

  const back = backend.map((elem) => {
    return <DisplayBox key={elem.id} item={elem} />
  })
    ...
    <div className=" px-[130px] py-5 pb-5">
      ...
      <div className="flex flex-col ">
        <div className="block my-10 bg-gray-800 rounded p-8 shadow-2xl">
          <h2 className="text-3xl pb-3 text-primary border-secondary uppercase text-gray-400 font-mono italic">
            Frontend
          </h2>
          <div className="grid grid-cols-7 items-end p-2 gap-2">{front}</div>
        </div>
        ...
        <div className="block my-10 bg-gray-800 rounded p-8 shadow-2xl">
          ...
          <div className="grid grid-cols-7 items-end p-2 gap-2">{back}</div>
        </div>
        ...
        <div className="block my-10 bg-gray-800 rounded p-8 shadow-2xl">
          ...
          <div className="grid grid-cols-7 items-end p-2 gap-2">{version}</div>
        </div>
        ...
        <div className="block my-10 bg-gray-800 rounded p-8 shadow-2xl">
          ...
          <div className="grid grid-cols-7 items-end p-2 gap-2">{others}</div>
        </div>
      </div>
    </div>
  )
