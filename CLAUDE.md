...
| New weekly log                                   | Add MDX to `content/weekly/<YYYY>-W<NN>.mdx` (ISO week) with `title`, `weekStart`, `weekEnd` frontmatter. Each rail entry (`read`/`watched`/`built`/`shipped`/`learned`/`met`) accepts a string OR `{text, href, image?, source?, kind?}`. YouTube URLs auto-derive thumbnails; common sources (GitHub, Substack, Medium, Latent Space, X, arXiv, Spotify) auto-fetch SimpleIcons logos via `lib/weekly-render.ts`. Latest log surfaces on the home page automatically. |
