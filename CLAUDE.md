...
| Route                                                  | Purpose                                                                   |
| ------------------------------------------------------ | ------------------------------------------------------------------------- |
| `/`                                                    | Hero · life arc · current focus · research · work · projects · writing    |
...
| `/skills.json`                                         | JSON feed of the skills library (`app/skills.json/route.ts`)              |
...
| `/sitemap.xml` · `/manifest.webmanifest`               | Native Next metadata routes (`app/sitemap.ts`, `app/manifest.ts`)         |
| `/robots.txt`                                          | Static file — `public/robots.txt`, not a metadata route                   |
...
- **Prod bundles**: `next.config.js` sets `experimental.optimizePackageImports` for barrel-heavy packages (`lucide-react`, `cmdk`) and `productionBrowserSourceMaps: false` to shorten build time and output size.
...
The capture script keeps a named browser session alive (`playwright-cli -s=weekly …`) so per-frame cost stays low. `scripts/capture-weekly.sh` is the reference for the full pipeline + tuning knobs.
...
| Surface                                          | How                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| ------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| New skill                                        | Add `content/skills/<slug>.md` with `name` + `description` frontmatter. **Exception:** the 8 session-workflow skills (`start` `wrap` `checkpoint` `mission` `map` `investigate` `featuredev` `harvest`) are generated — edit `~/.claude/commands/<slug>.md` and run `claude-sync` instead.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| New writing post                                 | Add MDX to `content/writing/*.mdx` with frontmatter.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| New weekly log                                   | Add MDX to `content/weekly/<YYYY>-W<NN>.mdx` (ISO week) with `title`, `weekStart`, `weekEnd` frontmatter. Each rail entry (`read`/`watched`/`built`/`shipped`/`learned`/`met`) accepts a string OR `{text, href, image?, source?, kind?, anchor?, bullets?, notes?, date?}`. `kind` must be one of: `youtube` · `podcast` · `article` · `paper` · `repo` · `meeting` · `tweet` · `note` (see `lib/weekly-types.ts`). Multiple entries per ISO week are allowed. YouTube URLs auto-derive thumbnails; common sources (GitHub, Substack, Medium, Latent Space, X, arXiv, Spotify) auto-fetch SimpleIcons logos via `lib/weekly-render.ts`. **Junk-filter rule for inbox triage:** only editorial-content newsletters and podcasts surface on `/weekly` — exclude marketing newsletters, OAuth/security-alert emails, and podcast-only promo messages. Render ASCII flows as fenced ` ``` ` code blocks inside the prose body (no new component needed). Latest logs surface on the home page rolling-log automatically. |
| New featured banner                              | Append to `BANNERS` in `lib/banners.ts`; highest `priority` and non-expired wins.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| New paper / experience / project / internal tool | Edit the relevant array in `lib/data.ts`. `Project` and `WorkTool` accept `links?: {label, href}[]` for multi-link surfaces (e.g. case study + walkthrough video) — rendered as a "References" section in `ProjectDetailModal`. Use the single `href` for the canonical destination and `links` for the rest.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
- Eight files in `content/skills/` are generated from `~/.claude/commands/*.md` by `claude-sync`
  (see the cheatsheet row above). Hand-edits to them are silently overwritten on the next sync —
  the "Session Workflow" category on `/skills` is exactly this set.
